...
void playerMovement(Player& player, float deltaTime, DashSmoke dashsmoke[100], playerBullets Bullets[10])
{
    ...
    //STATE RESOLUTION (final — only set state ONCE at the end)
    bool isShooting = Keyboard::isKeyPressed(Keyboard::Space);

    if (player.isDashing) {
        player.state = isShooting ? DASHSHOOTING : DASHING;
    }
    else if (!player.isOnGround) {
        player.state = isShooting ? JUMPSHOOTING : JUMPING;
    }
    else if (abs(player.velocity.x) > 1.0f) {
        player.state = isShooting ? RUNSHOOTING : RUNNING;
    }
    else {
        player.state = isShooting ? IDLESHO : STANDING;
    }
}